	...
	if(date_diff($dateDepart,$dateArrivee)->format("%R%a")<=0){
		...
		if($resultat[0]>0){
			...
			if($resultat[0]>0){
				...
				try {
					$reqInsertion = $bdd->prepare('UPDATE reservations SET dateArrivee = ?, dateDepart=?, numeroChambre_fk=?, numeroUtilisateur_fk=? WHERE numeroReservation = ?');
					$e=$reqInsertion->execute(array($dateArrivee->format('Y-m-d H:i:s'),$dateDepart->format('Y-m-d H:i:s'),$chambre,$utilisateur, htmlspecialchars($_POST['id'])));
				} catch (Exception $e) {
